...
namespace Geis.Locomotion
{
    ...
    public partial class GeisPlayerAnimationController : MonoBehaviour
    {
        ...
        /// <summary>Optional leaf state used for the double-tap forward roll (generated by the Setup Forward Roll menu).</summary>
        private static readonly int _dodgeLeafForwardRollHash = Animator.StringToHash("Dodge_Forward_Roll");
        /// <summary>DodgeDirection value that selects <c>Dodge_Forward_Roll</c> in the animator; matches the editor setup.</summary>
        private const int ForwardRollDodgeDirection = 4;
        ...
        [Tooltip("Max seconds between two dodge presses to count as a double-tap (triggers the forward roll variant).")]
        [SerializeField]
        private float _dodgeDoubleTapWindow = 0.25f;
        [Tooltip("If true, double-tapping dodge performs a forward roll using the Dodge_Back clip played in reverse. Disable to always use the normal sidestep dodges.")]
        [SerializeField]
        private bool _dodgeDoubleTapRollEnabled = true;
        [Tooltip("Multiplier applied to the forward-roll's horizontal root-motion travel. 1 = baked distance; >1 rolls further; <1 rolls shorter. Vertical (gravity) travel is never scaled.")]
        [Range(0.25f, 3f)]
        [SerializeField]
        private float _rollDistanceMultiplier = 1f;
        ...
        /// <summary>
        /// Unscaled time at which a pending first-tap dodge should commit to a (non-roll) dash. -1 = no pending dodge.
        /// Set by <see cref="OnDodgeRequested"/> on the first tap; cleared either by <see cref="CheckPendingDodgeCommit"/>
        /// once the window elapses, by a second tap upgrading to a roll, or by state changes away from Locomotion.
        /// </summary>
        private float _dodgePendingCommitAt = -1f;
        /// <summary>One-shot request consumed by <see cref="EnterDodgeState"/>; set by whichever path triggers the next dodge to request a reverse-back forward roll.</summary>
        private bool _dodgeRequestIsRoll;
        /// <summary>True while the current Dodge state is playing the reverse-baked Dodge_Forward_Roll clip.</summary>
        private bool _dodgeIsReverseRoll;
        ...
        private void OnDodgeRequested()
        {
            ...
            // Double-tap detection: second press within the window upgrades/triggers the forward-roll variant.
            float nowUnscaled = Time.unscaledTime;
            float dtSinceLast = _lastDodgeTapAt > 0f ? (nowUnscaled - _lastDodgeTapAt) : -1f;
            bool isDoubleTap = _dodgeDoubleTapRollEnabled
                && _lastDodgeTapAt > 0f
            ...
            {
                Debug.Log(
                    $"[GeisPlayerAnimationController] Dodge press — state={_currentState} dtSinceLast={dtSinceLast:F3}s window={_dodgeDoubleTapWindow:F3}s rollEnabled={_dodgeDoubleTapRollEnabled} doubleTap={isDoubleTap} pending={_dodgePendingCommitAt >= 0f}",
                    this);
            }
            ...
            if (_currentState == AnimationState.Attack)
            {
                ...
                _dodgeInputBufferIsRoll = isDoubleTap;
                return;
            }

            // Already dodging: further presses are ignored — a dodge/roll commits until its animation completes.
            ...
            // Rolls disabled → fire immediately with no wait; preserves zero-lag dashes when the feature is off.
            if (!_dodgeDoubleTapRollEnabled)
            {
                _dodgePendingCommitAt = -1f;
                _dodgeRequestIsRoll = false;
            ...
            // Second tap within the window → commit as a roll right now; cancel the pending dash so we don't double-fire.
            if (_dodgePendingCommitAt >= 0f && isDoubleTap)
            {
                _dodgePendingCommitAt = -1f;
                _dodgeRequestIsRoll = true;
                SwitchState(AnimationState.Dodge);
                return;
            }

            // First tap: schedule a dash commit once the double-tap window elapses. No animation plays yet — this
            // is what avoids the "dash starts, then snaps into a roll" pop when the user was actually double-tapping.
            _dodgePendingCommitAt = nowUnscaled + _dodgeDoubleTapWindow;
        }

        /// <summary>
        /// Polls each frame from <see cref="Update"/>. If a pending (first-tap) dodge's wait window has elapsed and
        /// the player is still in Locomotion, grounded, and not crouching, commits it as a normal dash. If any of
        /// those preconditions fail at expiry (airborne, crouching, mid-attack, etc.) the pending request is silently
        /// cancelled so it can't fire a stale dodge after a state change.
        /// </summary>
        private void CheckPendingDodgeCommit()
        {
            if (_dodgePendingCommitAt < 0f)
                return;
            if (Time.unscaledTime < _dodgePendingCommitAt)
                return;

            bool canCommit = _currentState == AnimationState.Locomotion
                && _isGrounded
                && !_isCrouching
                && !GeisInteractInput.IsMovementFrozenForInteraction
        ...
        private void EnterDodgeState()
        {
            ...
            // Consume the "next dodge should be a roll" request that the caller set before SwitchState(Dodge).
            // Keeping request and active flag separate prevents ExitDodgeState from clobbering the intent when a
            // double-tap triggers SwitchState(Dodge) from inside the Dodge state (mid-dodge upgrade path).
            _dodgeIsReverseRoll = _dodgeRequestIsRoll;
            _dodgeRequestIsRoll = false;

            // The forward roll state is optional (needs the editor setup menu). If it's missing, downgrade to a
            // normal forward dash so the double-tap still does something instead of throwing or no-op'ing.
            if (_dodgeIsReverseRoll && _animator != null && !_animator.HasState(0, _dodgeLeafForwardRollHash))
            {
                if (!_loggedForwardRollMissing)
                {
                    _loggedForwardRollMissing = true;
                    Debug.LogWarning(
                        "[GeisPlayerAnimationController] Animator is missing the 'Dodge_Forward_Roll' state. " +
                        "Run menu: Geis → Animator → Setup Forward Roll (Reverse-baked Dodge_Back). " +
                        "Falling back to a normal forward dash for this double-tap.",
                        this);
                }
                _dodgeIsReverseRoll = false;
            }

            // "Dash in the direction the character is facing" while running/walking, and always forward-roll on
            // double-tap. Idle (no movement input) keeps the stick-relative 4-way sidestep so players can still
            // back-step / side-step when stationary.
            bool isMoving = IsDodgingFromMovement();
            bool useCharacterFacing = _dodgeIsReverseRoll || isMoving;

            int dir;
            Vector3 faceWorld;
            if (_dodgeIsReverseRoll)
            {
                // Sentinel direction index — the animator's Any-State transition routes this to Dodge_Forward_Roll
                // (a reverse-baked copy of Dodge_Back that plays forward at speed 1, avoiding the Unity editor's
                // "Animator.speed can only be negative when recorder is enabled" restriction).
                dir = ForwardRollDodgeDirection;
                faceWorld = GetPlanarForward();
            }
            else if (isMoving)
            {
                dir = 0;
            ...
            {
                dir = ComputeDodgeDirectionIndex();
                faceWorld = GetDodgeFacingWorld(dir);
            }

            // Strafing preserves yaw only for the idle 4-way dodge. Movement dashes and reverse rolls always commit
            // to the character's facing (in strafe that's already camera-forward, so no visible rotation happens).
            _dodgePreserveStrafeFacing = !useCharacterFacing && _isStrafing;
        ...
        /// <summary>True when the current move-stick input is outside the dodge deadzone (i.e. the player is walking/running/sprinting).</summary>
        private bool IsDodgingFromMovement()
        {
            if (_inputReader == null) return false;
            Vector2 m = GeisInteractInput.GetEffectiveMoveCompositeForLocomotion(_inputReader._moveComposite);
            return m.sqrMagnitude >= _dodgeInputDeadzone * _dodgeInputDeadzone;
        }

        /// <summary>Character's planar (Y-zeroed) forward vector, normalized. Falls back to world <see cref="Vector3.forward"/> if degenerate.</summary>
        ...
        private static bool IsDodgeLeafShortNameHash(int shortNameHash)
        {
            return shortNameHash == _dodgeLeafFrontHash || shortNameHash == _dodgeLeafBackHash
                ...
                || shortNameHash == _dodgeLeafForwardRollHash;
        }
        ...
        private void UpdateDodgeState()
        {
            ...
            if (_animator != null)
            {
                ...
                bool inRecoveryWindow = _dodgeAnimatorEnteredLeaf
                    ...
                    && info.normalizedTime >= _dodgeRecoveryStartNormalizedTime;
        ...
        private void OnAnimatorMove()
        {
            ...
            else if (_currentState == AnimationState.Dodge)
            {
                ...
                // Scale only the horizontal component of the forward roll's root motion; leave side-step/dash dodges
                // untouched so this field affects roll distance specifically. Vertical (gravity) is added AFTER the
                // scale so the player doesn't get lifted/dropped when multiplier != 1.
                if (_dodgeIsReverseRoll && !Mathf.Approximately(_rollDistanceMultiplier, 1f))
                {
        ...
        private void Update()
        {
            ...
            // Fire deferred dodge commits before the state-switch dispatch so the newly-entered Dodge state's
            // UpdateDodgeState can run this same frame (matches the behavior of any other in-frame SwitchState call).
            CheckPendingDodgeCommit();
