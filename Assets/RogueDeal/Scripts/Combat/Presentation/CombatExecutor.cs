...
namespace RogueDeal.Combat.Presentation
{
    ...
    public class CombatExecutor : MonoBehaviour
    {
        ...
        /// <summary>
        /// Triggers hit reaction animation and damage popup on the target
        /// Can be called from WeaponHitbox when collision-based hits occur
        /// </summary>
        public void TriggerHitReaction(CombatEntity target, float damageDealt, bool isCritical)
        {
            if (target == null) return;
            
            // Get hit position (for damage popup)
            Vector3 hitPosition = target.hitPoint != null 
                ? target.hitPoint.position 
                : target.transform.position + Vector3.up;
            
            // Try to use CombatAnimationController for hit reaction
            ...
            // Try to get EnemyVisual component
            EnemyVisual enemyVisual = target.GetComponent<EnemyVisual>();
            if (enemyVisual != null)
            {
                // Show damage popup and visual feedback
                enemyVisual.AnimateDamage(Mathf.RoundToInt(damageDealt), isCritical);
                return;
            }
            
            // Try to get PlayerVisual component
            PlayerVisual playerVisual = target.GetComponent<PlayerVisual>();
            if (playerVisual != null)
            {
                // Show damage popup and visual feedback
                playerVisual.AnimateDamage(Mathf.RoundToInt(damageDealt));
                return;
            }
            
            // Fallback: Try to get visual components from parent
            enemyVisual = target.GetComponentInParent<EnemyVisual>();
            if (enemyVisual != null)
            {
                enemyVisual.AnimateDamage(Mathf.RoundToInt(damageDealt), isCritical);
                return;
            ...
            playerVisual = target.GetComponentInParent<PlayerVisual>();
            if (playerVisual != null)
            {
            ...
            // Fallback: Try to use CombatAnimationController if available
            var animationController = target.GetComponent<CombatAnimationController>();
            if (animationController != null)
            {
                animationController.PlayHitReaction(EffectType.Damage);
            }
            
            // Fallback: Use DamagePopupManager directly if no visual component found
